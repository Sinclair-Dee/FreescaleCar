...
void Ov7725_exti_Init()
{
    ...
    DMA_PORTx2BUFF_Init(CAMERA_DMA_CH, (void *)&PTB_BYTE2_IN, (void *)IMG_BUFF, PTC8, DMA_BYTE1, CAMERA_SIZE , KEEPON);
    port_init(PTC8, DMA_RISING | PULLUP );   //PCLK

    DMA_IRQ_EN(DMA_CH0);

    port_init(PTB3, IRQ_RISING | PULLUP | PF);   //场中断，下拉，下降沿触发中断，带滤波
